...
export default function Hero() {
  ...
  return (
    <>
              ...
              <div className="flex flex-col justify-center p-6 text-center rounded-sm lg:max-w-md xl:max-w-lg lg:text-left">
                ...
                <div className="flex flex-col space-y-4 sm:items-center sm:justify-center sm:flex-row sm:space-y-0 sm:space-x-4 lg:justify-start">
                  <button
                    ...
                    onClick={() => setLoginRender(true)}
                  >
                    Login
                  </button>
                  <button
                    class="inline-flex items-center justify-center px-5 py-3 text-base font-medium text-center text-gray-900 border border-gray-300 rounded-lg hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 dark:text-white dark:border-gray-700 dark:hover:bg-gray-700 dark:focus:ring-gray-800"
                    onClick={() => setSignupRender(true)}
                  >
                    Register
                  </button>
                </div>
      ...
      {loginRender && <Login render={setLoginRender} signupRender={signupRender} setSignupRender={setSignupRender} />}

      {signupRender && <Signup render={setSignupRender} loginRender={loginRender} setLoginRender={setLoginRender} />}
    </>
  );
}